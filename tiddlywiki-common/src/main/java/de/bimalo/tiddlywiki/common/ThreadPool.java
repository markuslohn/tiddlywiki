...
/**
 * <p>
 * Implements a pool for threads. The maximum number of thread in the ThreadPool can be defined when
 * the ThreadPool gets created. In the <code>run</code> function a <code>Runnable</code>
 * implementation can be provided that will be executed by a thread in this ThreadPool. When the
 * <code>shutdown</code> function was called all threads in this ThreadPool will be shutdown when
...
public final class ThreadPool {
  ...
  /**
   * Constructs a new <code>ThreadPool</code>.
   *
   * @param capacity how many threads in the thread pool will be initially started.
   */
  public ThreadPool(int capacity) {
    ...
    for (int i = 0; i < this.capacity; i++) {
      WorkerThread worker = new WorkerThread("WorkerThread_" + i, this);
      worker.start();
      waitingThreads.push(worker);
    }
    this.activeThreads = this.capacity;

    logger.info("ThreadPool with {} WorkerThreads initialized.", String.valueOf(this.capacity));
  }

  /**
   * <p>
   * Executes a task provided by a class that implements the <code>Runnable</code> interface.
   * </p>
   * <p>
   * It uses an already started thread from this ThreadPool to execute the task. If currently no
   * thread is available in the ThreadPool it creates a new thread to execute the task.
   * </p>
   *
  ...
  public void run(Runnable target) {
    ...
    synchronized (waitingThreads) {
      if (waitingThreads.empty()) {
        if (!shutdown) {
          worker = new WorkerThread("WorkerThread_" + this.activeThreads, this);
          this.activeThreads++;
        } else {
          logger.info(
              "ThreadPool has been shutdown. Due to this new worker threads cannot be started!");
        }
      } else {
        worker = (WorkerThread) waitingThreads.pop();
      }
      logger.trace("Active WorkerThread count= {}.", String.valueOf(this.activeThreads));
    }
    if (worker != null) {
      worker.activate(target);
    }
  }

  /**
   * Shutdowns the ThreadPool. This means all started threads by this ThreadPool will be shutdown
  ...
  /**
   * Push back the finished thread into this ThreadPool when the capacity of the pool has not
   * reached.
   *
   * @param worker the WorkerThread finished processing and can be pushed back in the pool.
   * @return true if true if pushed back in the pool otherwise false
   * @IllegalArgumentException if w is null
   */
  private synchronized boolean push(WorkerThread worker) {
    Assert.notNull(worker);
    boolean result = false;
    if (this.waitingThreads.size() < this.capacity && !shutdown) {
      this.waitingThreads.push(worker);
      notify();
      result = true;
    } else {
      this.activeThreads--;
  ...
  private class WorkerThread extends Thread {
    ...
    public synchronized void activate(Runnable target) {
      ...
      this.target = target;
      notify();
      logger.trace("WorkerThread {} has been activated.", this.identifier);
    }
      ...
      while (!stop) {
        if (target == null) {
          try {
            ...
            logger.error("WorkerThread " + this.identifier + " reports an error.", e);
            continue;
          }
        }
        if (target != null) {
